...
class HPatrolSingaporeStack(Stack):
    ...
    def __init__(self,
        ...
        **kwargs) -> None:
        ...
        generators = ["gandongyun"]
        csf.createLambdasStructure(self, gensDir, generators, executionMode)

        self._createGandongyunLambda("gandongyun")


    def _createGandongyunLambda(self, buildDir) -> str:
        theLambda = PythonFunction(self, "gandongyunLambda",
            description="Collects and parses the gandongyun site to create aimpoints",
            ...
            function_name=self.baseStackName + "_Gandongyun",
            memory_size=1024,
        ...
        logger.info("Gandongyun lambda defined")
        ...
        # Running every day at 0605UTC
        cronRule = Rule(
            self, "GandongyunRule",
            schedule=Schedule.cron(
                minute="05",
                hour="06", # 6 AM UTC, 2 AM EST
                # day="?",
                month="*",
                week_day="*",
                year="*"),
            enabled=False   # Disabled since access was lost
        )
